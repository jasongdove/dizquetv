...
module.exports = function ($scope, $timeout, dizquetv) {
    ...
    $scope.refreshShow = async () => {
        ...
        $timeout();
        const shows = await dizquetv.getAllShowsInfo();
        $scope.shows = shows;
        $timeout();
    };
    $scope.refreshShow();
    ...
    $scope.selectShow = async (index) => {
        try {
            ...
            if (index === -1) {
                ...
                const f = await dizquetv.getShow($scope.shows[index].id);
                feedToShowConfig(f);
                $timeout();
            }
        } catch (err) {
    ...
    $scope.deleteShow = async (index) => {
        try {
            ...
            const show = $scope.shows[index];
            if (
                confirm(
                    "Are you sure to delete show: " +
                        show.name +
                        "? This will NOT delete the show's programs from channels that are using.",
                )
            ) {
                show.pending = true;
