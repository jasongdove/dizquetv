...
export default function (getShowData) {
    ...
    function sortShows(programs) {
        ...
        for (let i = 0, l = progs.length; i < l; i++) {
            const showData = getShowData(progs[i]);
        ...
        for (let i = 0, l = keys.length; i < l; i++) {
            shows[keys[i]].sort((a, b) => {
                const aData = getShowData(a);
                const bData = getShowData(b);
    ...
    const removeDuplicates = (progs) => {
        ...
        const newProgs = [];
        const keys = Object.keys(tmpProgs);
        for (let i = 0, l = keys.length; i < l; i++) {
    ...
    const removeSpecials = (progs) => {
        const tmpProgs = [];
        for (let i = 0, l = progs.length; i < l; i++) {
            if (typeof progs[i].customShowId !== "undefined" || progs[i].season !== 0) {
    ...
    const getProgramDisplayTitle = (x) => {
        let s =
            x.type === "episode"
                ? x.showTitle +
                  " - S" +
                  x.season.toString().padStart(2, "0") +
                  "E" +
                  x.episode.toString().padStart(2, "0")
                : x.title;
    ...
    const sortByDate = (programs) => {
        programs.sort((a, b) => {
            const aHas = typeof a.date !== "undefined";
            const bHas = typeof b.date !== "undefined";
            if (!aHas && !bHas) {
                return 0;
            } else if (!aHas) {
                return 1;
    ...
    const programSquareStyle = (program) => {
        let background = "";
        if (program.isOffline && program.type !== "redirect") {
            ...
            if (program.type === "redirect") {
                ...
                const h = Math.abs(getHashCode(program.customShowId, false));
                const h2 = Math.abs(getHashCode(program.customShowId, true));
                r = h % 256;
                g = (h / 256) % 256;
                b = (h / (256 * 256)) % 256;
                r2 = (h2 / (256 * 256)) % 256;
                ...
                const h = Math.abs(getHashCode(program.showTitle, false));
                const h2 = Math.abs(getHashCode(program.showTitle, true));
                r = h % 256;
                g = (h / 256) % 256;
                b = (h / (256 * 256)) % 256;
                r2 = (h2 / (256 * 256)) % 256;
