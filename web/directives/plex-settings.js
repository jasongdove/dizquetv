    ...
    return {
        ...
        link(scope, element, attrs) {
            ...
            scope.refreshServerList = async () => {
                scope.serversPending = true;
                const servers = await dizquetv.getPlexServers();
            ...
            scope.isAnyUIBad = () => {
                ...
                if (scope.servers) {
                    for (let i = 0; i < scope.servers.length; i++) {
                        const s = scope.servers[i];
                        if (s.uiStatus == -1 || (s.uiStatus == 0 && s.uiPending + 30000 < t)) {
                            return true;
                        }
                    }
                }
            ...
            scope.isAnyBackendBad = () => {
                ...
                if (scope.servers) {
                    for (let i = 0; i < scope.servers.length; i++) {
                        const s = scope.servers[i];
                        if (s.backendStatus == -1 || (s.backendStatus == 0 && s.backendPending + 30000 < t)) {
                            return true;
                        }
                    }
                }
            ...
            scope.refreshUIStatus = async (t, i) => {
                const s = await plex.check(scope.servers[i]);
            ...
            scope.refreshBackendStatus = async (t, i) => {
                const s = await dizquetv.checkExistingPlexServer(scope.servers[i].name);
                ...
                await Promise.any(
                    connections.map(async (connection) => {
                        ...
                        const q = await Promise.race([
                            ...
                            (async () => {
                                const s1 = await plex.check(hypothethical);
