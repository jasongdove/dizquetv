        ...
        link: {
            post(scope, element, attrs) {
                ...
                if (typeof scope.channel === "undefined" || scope.channel == null) {
                    ...
                    if (scope.channel.startTime.getMinutes() < 30) scope.channel.startTime.setMinutes(0);
                    ...
                    if (typeof scope.channel.watermark === "undefined" || scope.channel.watermark.enabled !== true) {
                        scope.channel.watermark = defaultWatermark();
                    }

                    if (typeof scope.channel.groupTitle === "undefined" || scope.channel.groupTitle === "") {
                        scope.channel.groupTitle = "dizqueTV";
                    }
                ...
                function adjustStartTimeToCurrentProgram() {
                    ...
                    const n = scope.channel.programs.length;
                    ...
                    if (
                        ...
                        const m = scope.channel.onDemand.firstProgramModulo;
                        const n = originalStart % scope.channel.onDemand.modulo;
                        if (n < m) {
                            originalStart += m - n;
                        } else if (n > m) {
                    ...
                    const m = (t - originalStart) % totalDuration;
                    let x = 0;
                    let runningProgram = -1;
                    let offset = 0;
                    for (let i = 0; i < n; i++) {
                        const d = scope.channel.programs[i].duration;
                        if (x + d > m) {
                            runningProgram = i;
                            offset = m - x;
                            break;
                ...
                const addMinuteVersionsOfFields = () => {
                ...
                const removeMinuteVersionsOfFields = (channel) => {
                ...
                scope.finishedOfflineEdit = (program) => {
                    const editedProgram = scope.channel.programs[scope.selectedProgram];
                    const duration = program.durationSeconds * 1000;
                ...
                scope.finishedAddingOffline = (result) => {
                    const duration = result.durationSeconds * 1000;
                    const program = {
                        duration,
                        isOffline: true,
                    };
                ...
                scope.sortByDate = () => {
                    scope.removeOffline();
                    scope.channel.programs = commonProgramTools.sortByDate(scope.channel.programs);
                    updateChannelDuration();
                };
                scope.slideAllPrograms = (offset) => {
                    const t0 = scope.channel.startTime.getTime();
                    let t1 = t0 - offset;
                    const t = new Date().getTime();
                    const total = scope.channel.duration;
                    while (t1 > t) {
                ...
                scope.removeOffline = () => {
                    const tmpProgs = [];
                    const progs = scope.channel.programs;
                ...
                scope.doReruns = (rerunStart, rerunBlockSize, rerunRepeats) => {
                    ...
                    const start = (o + rerunStart * 60 * 60 * 1000) % (24 * 60 * 60 * 1000);
                    const blockSize = rerunBlockSize * 60 * 60 * 1000;
                    const repeats = rerunRepeats;
                    ...
                    const addBlock = () => {
                        ...
                        const m = high % blockSize;
                        if (m >= 1000) {
                            high = high - m + blockSize;
                        }
                        high -= currentSize;
                        const rem = Math.max(0, high - currentBlockSize);
                        if (rem >= 1000) {
                            currentBlockSize += rem;
                            const t = block.length;
                            if (t > 0 && block[t - 1].isOffline && block[t - 1].type !== "redirect") {
                    ...
                    scope.channel.startTime = new Date(
                        scope.channel.startTime.getTime() - (scope.channel.startTime % (24 * 60 * 60 * 1000)) + start,
                    );
                ...
                scope.savePositions = () => {
                    ...
                    const array = scope.channel.programs;
                    for (let i = 0; i < array.length; i++) {
                        const data = getShowData(array[i]);
                        if (data.hasShow) {
                            const key = data.showId;
                ...
                scope.recoverPositions = () => {
                    ...
                    for (let i = 0; i < array.length; i++) {
                        const vid = array[i];
                        const data = getShowData(vid);
                        if (data.hasShow) {
                            const countKey = {
                                id: data.showId,
                                order: data.order,
                            };
                            const key = JSON.stringify(countKey);
                            const c = typeof counts[key] === "undefined" ? 0 : counts[key];
                            counts[key] = c + 1;
                            const showEntry = {
                                c,
                                it: vid,
                    ...
                    Object.keys(shows).forEach((key, index) => {
                        shows[key].sort((a, b) => {
                            if (a.c == b.c) {
                        ...
                        if (typeof scope.episodeMemory[key] !== "undefined") {
                            for (let i = 0; i < shows[key].length; i++) {
                                if (getShowData(shows[key][i].it).order == scope.episodeMemory[key]) {
                                    next[key] = i;
                                    break;
                                }
                            }
                ...
                scope.cannotRecoverPositions = () => scope.episodeMemory.saved !== true;

                scope.addBreaks = (afterMinutes, minDurationSeconds, maxDurationSeconds) => {
                    const after = afterMinutes * 60 * 1000 + 5000; //allow some seconds of excess
                    const minDur = minDurationSeconds;
                    const maxDur = maxDurationSeconds;
                    const progs = [];
                    let tired = 0;
                    for (let i = 0, l = scope.channel.programs.length; i <= l; i++) {
                        const prog = scope.channel.programs[i % l];
                        if (prog.isOffline && prog.type != "redirect") {
                            tired = 0;
                        } else {
                            if (tired + prog.duration >= after) {
                                tired = 0;
                                const dur = 1000 * (minDur + Math.floor((maxDur - minDur) * Math.random()));
                                progs.push({
                                    isOffline: true,
                                    duration: dur,
                                });
                ...
                scope.padTimes = (paddingMod, allow5) => {
                    ...
                    for (let i = 0, l = scope.channel.programs.length; i < l; i++) {
                        const prog = scope.channel.programs[i];
                        progs.push(prog);
                        t += prog.duration;
                        addPad(i == l - 1);
                    }
                ...
                scope.blockShuffle = (blockCount, randomize) => {
                    ...
                    const progs = scope.channel.programs;
                    for (let i = 0, l = progs.length; i < l; i++) {
                        const data = getShowData(progs[i]);
                        if (!data.hasShow) {
                            continue;
                    ...
                    while (keys.length > 0) {
                        if (shows[keys[index]].length === 0) {
                            ...
                            if (randomize) {
                                const tmp = index;
                                index = getRandomInt(0, keys.length - 1);
                                while (keys.length > 1 && tmp == index) index = getRandomInt(0, keys.length - 1);
                            } else if (index >= keys.length) index = 0;
                            continue;
                        }
                        for (let i = 0, l = blockCount; i < l; i++) {
                            if (shows[keys[index]].length > 0) newProgs.push(shows[keys[index]].shift());
                        }
                        if (randomize) {
                            const tmp = index;
                            index = getRandomInt(0, keys.length - 1);
                ...
                scope.cyclicShuffle = () => {
                    ...
                    const oldSaved = scope.episodeMemory;
                    commonProgramTools.shuffle(scope.channel.programs);
                    scope.savePositions();
                    scope.recoverPositions();
                ...
                scope.startFrequencyTweak = () => {
                    ...
                    const displayName = {};
                    for (let i = 0; i < scope.channel.programs.length; i++) {
                        const data = getShowData(scope.channel.programs[i]);
                        if (data.hasShow) {
                            const c = data.showId;
                    ...
                    const arr = [];
                    Object.keys(programs).forEach((key, index) => {
                        const w = Math.ceil((24.0 * programs[key]) / mx);
                        const obj = {
                            name: key,
                            weight: w,
                ...
                function equalizeShows(array, freqObject) {
                    ...
                    let good = true;
                    Object.keys(shows).forEach((key, index) => {
                        const amount = Math.floor((maxDuration * F) / shows[key].total);
                ...
                scope._onDone = async (channel) => {
                    if (typeof channel === "undefined") {
                        ...
                        if (typeof channel.number === "undefined" || channel.number === null || channel.number === "") {
                        ...
                        } else if (channelNumbers.indexOf(parseInt(channel.number, 10)) !== -1 && scope.isNewChannel) {
                            ...
                            channel.watermark.enabled &&
                            scope.channel.watermark.width + scope.channel.watermark.horizontalMargin > 100.0
                        ) {
                            ...
                            scope.error.tab = "ffmpeg";
                        } else if (channel.offlineMode != "pic" && channel.fallback.length == 0) {
                            ...
                            try {
                                ...
                                if (s.length > 50 * 1000 * 1000) {
                                    ...
                                    cloned.fillerCollections = cloned.fillerCollections.filter((f) => f.id != "none");
                ...
                scope.selectProgram = (index) => {
                    ...
                    const program = scope.channel.programs[index];
                    ...
                    try {
                        await Promise.all(
                            channelNumbers.map(async (x) => {
                                const desc = await dizquetv.getChannelDescription(x);
                ...
                scope.hasPrograms = () => scope.channel.programs.length > 0;
                ...
                scope.disablePadding = () =>
                    scope.paddingOption.id == -1 || 2 * scope.channel.programs.length > scope.maxSize;
                ...
                scope.resolutionOptions = [{ id: "", description: "(Use global setting)" }];
                ...
                scope.atNightEnd = -1;
                for (let i = 0; i < 24; i++) {
                    const v = { id: i, description: (i < 10 ? "0" : "") + i + ":00" };
                ...
                scope.programmingHeight = () => scope.programming.maxHeight + "rem";
                const setProgrammingHeight = (h) => {
                    scope.programming.step++;
                    $timeout(() => {
                        scope.programming.step--;
                    }, 1000);
                    scope.programming.maxHeight = h;
                    localStorage.setItem("channel-programming-list-height", String(h));
                };
                    ...
                    for (let i = 0; i < scope.channel.fillerCollections.length; i++) {
                        if (scope.channel.fillerCollections[i].id !== "none") {
                            scope.channel.fillerCollections[i].percentage =
                                ((scope.channel.fillerCollections[i].weight * 100) / w).toFixed(2) + "%";
                        }
                    }
                };
                ...
                const addAddFiller = () => {
                    if (
                        ...
                        scope.channel.fillerCollections[scope.channel.fillerCollections.length - 1].id !== "none"
                    ) {
                ...
                const refreshIndividualOptions = () => {
                    for (let i = 0; i < scope.channel.fillerCollections.length; i++) {
                        scope.channel.fillerCollections[i].options = fillerOptionsFor(i);
                    }
                };

                const refreshFillerOptions = async () => {
                    try {
                        const r = await dizquetv.getAllFillersInfo();
                ...
                const refreshScreenResolution = async () => {
                    try {
                        const ffmpegSettings = await dizquetv.getFfmpegSettings();
                        if (
                ...
                scope.showList = () => !scope.showFallbackPlexLibrary;
                ...
                scope.getWatermarkPreviewOuter = () => {
                    const tm = scope.getCurrentWH();
                    const resolutionW = tm.w;
                    const resolutionH = tm.h;
                    const width = 100;
                    const height = width / (resolutionW / resolutionH);
                ...
                scope.getWatermarkSrc = () => {
                    let { url } = scope.channel.watermark;
                ...
                scope.getWatermarkPreviewInner = () => {
                    const width = Math.max(Math.min(100, scope.channel.watermark.width), 0);
                    const res = {
                        width: `${width}%`,
                        margin: "0",
                        position: "absolute",
                    ...
                    const mH = scope.channel.watermark.horizontalMargin;
                    const mV = scope.channel.watermark.verticalMargin;
                ...
                const readSlotsResult = (slotsResult) => {
                ...
                scope.onTimeSlotsButtonClick = () => {
                    const progs = commonProgramTools.removeDuplicates(scope.channel.programs);
                ...
                scope.rerollRandomSlots = () => {
                    const progs = commonProgramTools.removeDuplicates(scope.channel.programs);
                    scope.randomSlots.startDialog(progs, scope.maxSize, scope.channel.randomScheduleBackup, true);
                };
                ...
                scope.rerollTimeSlots = () => {
                    const progs = commonProgramTools.removeDuplicates(scope.channel.programs);
                    scope.timeSlots.startDialog(progs, scope.maxSize, scope.channel.scheduleBackup, true);
                };
                scope.hasNoTimeSlots = () =>
                    typeof scope.channel.scheduleBackup === "undefined" || scope.channel.scheduleBackup == null;
                ...
                scope.hasAdvancedTools = () => scope.advancedTools;
