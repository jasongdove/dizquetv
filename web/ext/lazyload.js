export default function (angular) {
    ...
    angular.module("angularLazyImg").factory("LazyImgMagic", [
        ...
        function ($window, $rootScope, lazyImgConfig, lazyImgHelpers) {
            ...
            const images = [];
            const options = lazyImgConfig.getOptions();
            const $win = angular.element($window);
            winDimensions = lazyImgHelpers.getWinDimensions();
            const saveWinOffsetT = lazyImgHelpers.throttle(function () {
                winDimensions = lazyImgHelpers.getWinDimensions();
            }, 60);
            options.container = options.containers || options.container;
            const containers = options.container ? [].concat(options.container) : [$win];
            ...
            const checkImagesT = lazyImgHelpers.throttle(checkImages, 30);
        ...
        .directive("lazyImg", [
            ...
            function ($rootScope, $log, LazyImgMagic) {
                ...
                function link(scope, element, attributes) {
                    ...
                    const eventsDeregister = $rootScope.$on("lazyImg:refresh", function () {
