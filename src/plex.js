    ...
    async Put(path, query = {}, optionalHeaders = {}) {
        ...
        await new Promise((resolve, reject) => {
            if (this._accessToken === "")
                ...
                request(req, (err, res) => {
                    if (err || res.statusCode !== 200) reject(`Plex 'Put' request failed. URL: ${this.URL}${path}`);
    ...
    Post(path, query = {}, optionalHeaders = {}) {
        ...
        return new Promise((resolve, reject) => {
            if (this._accessToken === "")
                ...
                request(req, (err, res) => {
                    if (err || res.statusCode !== 200) reject(`Plex 'Post' request failed. URL: ${this.URL}${path}`);
                    else resolve(res.body);
                });
        });
    }
...
module.exports = { Plex, redirectRouter };
